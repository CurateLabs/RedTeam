...
- **Observability:** No runtime telemetry; review artifacts in `.redteam/reviews/` serve as the audit trail

## Decisions

Significant architectural choices are recorded as ADRs in [`3-ENGINEERING/ADRs/`](3-ENGINEERING/ADRs/). Create the next one with `docslime add adr <short-slug>`.
...
| Handbook techniques summarized, not reproduced verbatim | Accepted, ADR pending | See [`SOURCE.md`](SOURCE.md) |
